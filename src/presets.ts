...
export function registerBuiltinModes(): void {
  ...
  modeRegistry.alias('working', 'orbits'); modeRegistry.alias('searching', 'globe'); modeRegistry.alias('solving', 'rubik')
  modeRegistry.alias('listening', 'wave'); modeRegistry.alias('connecting', 'web'); modeRegistry.alias('weaving', 'braid')
...
export function resolvePreset(state: OrbState, size: number): { mode: AnimationMode | undefined; speed: number; density: number; opts: Record<string, number> } {
  const mode = modeRegistry.get(state); const point = DENSITY_PRESETS[state] ?? DENSITY_PRESETS.working
...
const DENSITY_PRESETS: Record<string, DensityPoint> = {
  working: { small: { speed: 3.9, density: 0.24 }, large: { speed: 1.89, density: 1.0 } },
  searching: { small: { speed: 2.67, density: 0.11, extra: { scanMul: 4.34, dimBase: 0.45 } }, large: { speed: 2.02, density: 0.42, extra: { scanMul: 4.08, dimBase: 0.45 } } },
  solving: { small: { speed: 1.95, density: 0.09 }, large: { speed: 1.82, density: 0.35 } },
  listening: { small: { speed: 4.0, density: 0.11 }, large: { speed: 4.39, density: 0.34 } },
  connecting: { small: { speed: 6.63, density: 0.25 }, large: { speed: 3.32, density: 1.35 } },
  weaving: { small: { speed: 2.75, density: 0.11 }, large: { speed: 1.63, density: 0.50 } },
  composing: { small: { speed: 3.12, density: 0.05, extra: { spin: 0, bandMul: 4.94, wobMul: 1 } }, large: { speed: 2.34, density: 0.25, extra: { spin: 0, bandMul: 3.9, wobMul: 1 } } },
  breathing: { small: { speed: 3.78, density: 0.03, extra: { spin: 0, bandMul: 3.97, wobMul: 0.57 } }, large: { speed: 3.24, density: 0.25, extra: { spin: 0, bandMul: 3.63, wobMul: 0.37 } } },
  shaping: { small: { speed: 2.08, density: 0.53, extra: { spread: 1.45 } }, large: { speed: 2.41, density: 0.70, extra: { spread: 1.45 } } },
}